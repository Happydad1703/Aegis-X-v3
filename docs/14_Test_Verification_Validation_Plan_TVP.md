...
________________________________________
13. Test & Evaluation Master Plan (TEMP) 연계
________________________________________
실행 순서·계약 테스트·Phase 0-1 필수 검증·RTM 요약·진입/완료 기준·지속 검증은 **docs/Test_Evaluation_Master_Plan.md** (TEMP)에서 통합 관리. TVP §1–§12는 검증 요구사항 정의, TEMP는 검증 실행 계획 및 현재 구현 매핑.
